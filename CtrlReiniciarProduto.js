...
module.exports = {
  configurar: async (srv) => {
    servidor = srv;
    // Resseta os Produtos e Marcas caso o path seja /reset e a requisição seja post
    servidor.post("/reset", module.exports.reiniciarProduto);
  },
  reiniciarProduto: async (request, reply) => {
    ...
    if (
      ...
      // Solicita a limpeza dos produtos.
      params.produtos = await db.limparProdutos();

      // Define a mensagem de erro, caso não tenha realizado a ação de limpeza com sucesso
      params.error = params.produtos ? null : data.errorMessage;
    }
